...
**Purpose:** Completely wipe and format disk before installation (for fresh installs). This is a destructive operation that prepares the entire disk for a new operating system. For general information on partitioning, refer to the [ArchWiki on Partitioning](https://wiki.archlinux.org/title/Partitioning). For securely wiping a disk, refer to the [ArchWiki on Securely wipe disk](https://wiki.archlinux.org/title/Securely_wipe_disk).
...
**For dual boot or preserving data:** Use `disk-partitioning.md` instead.
